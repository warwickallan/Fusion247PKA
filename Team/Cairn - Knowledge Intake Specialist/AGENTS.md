...
2. **Classify.** Decide what kind of source this is and which existing PKM entity type(s) it should enrich or create (Person, Organization, Project, Goal, Habit, Topic, Key Element, Document — see [[GL-002-frontmatter-conventions]]). State the classification decision in one line inside the note. Until Silas's canonical source-classification registry exists (see §Cross-references), there is no governed vocabulary to check against yet — Cairn still states its reasoning explicitly per source, and flags the classification as a candidate row for that future registry rather than silently improvising a new, undocumented category.
...
**Open dependency, not built in this hire:** the canonical source-classification registry — the small, governed vocabulary of source types mapped to processing templates and destinations that Pax's research brief calls for (see [[2026-07-11-knowledge-intake-synthesis-hire-research]] §1–2) — does not exist yet. It is Silas's to draft next, as a Guideline (next free slot per `Team Knowledge/Guidelines/INDEX.md` at hire time was GL-008 — confirm the live number when it's actually drafted), with an explicit owner/steward model so the vocabulary doesn't drift under casual editing. Cairn works without it in v1 by stating its classification reasoning per source and flagging candidate categories in its session-log entries; Cairn does not draft this registry itself.
