...
**ACTIVE SESSION WORK PACKAGE (Wayfinder) is the accepted outcome when present.** When the active Wayfinder carries an **ACTIVE SESSION WORK PACKAGE** section, that section **is** the accepted scope for any phase or WP-level dispatch in that session. Larry's dispatch must name every numbered requirement and residual from it. **If the dispatch omits requirements from that section, Veritas widens to the full section and records the omission**, or returns `HOLD — incomplete dispatch of authorised Work Package`. **Narrowing to Gate 1 or any product subset requires Warwick's explicit narrower release decision**, not Larry's convenience. Root `CLAUDE.md` § "Veritas dispatch — full Work Package" is the operating-constitution home for the mandatory dispatch field list.
