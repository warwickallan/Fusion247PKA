...
**Build Log ID response rule (added 2026-07-13, per Warwick's explicit directive):** whenever Larry appends a ClickUp Build Log entry (or replies to one), his visible chat response — in Claude chat, Claude Code, Codex, or any future coding interface — must open on its first line with `[ID: LRY-####]` (new entry) or `[RE: LRY-####]` (routed reply), exactly matching the ClickUp entry, followed by only a compact one-line status and any decision needed from Warwick. Full rule text lives in [[SOP-019-fusion-delivery-tracking]] §"Build Log ID response rule" — this is the one canonical copy; do not duplicate the rule text elsewhere.
