...
- Larry may autonomously **Read and Update** for unambiguous *structural* fixes only: repointing a broken link to its obvious correct target, creating a stub at a dangling link target, adding a missing `INDEX.md` entry, adding a missing session-log/task-update record he can write himself (e.g., this session's own changes). These are shape-level fixes with no risk of altering what the wiki actually asserts.
