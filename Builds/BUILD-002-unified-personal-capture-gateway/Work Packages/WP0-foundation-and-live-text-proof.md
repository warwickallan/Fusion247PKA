...
The offline-safe "no false completion" guarantee depends on atomicity across durable-enqueue → governed Markdown write → evidence-pointer → card edit. "Durably queued" is the commit point; the card is a retried projection; the Markdown write is idempotent. See the invariant section in [[BUILD-002-unified-personal-capture-gateway]].
