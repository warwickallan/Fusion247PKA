...
## Load-bearing WP0 invariant (recorded — Larry enforces in build)

Silas and Mack independently converged on the single most fragile point: the **offline-safe "no false completion" guarantee depends on atomicity** across durable-enqueue → governed Markdown write → evidence-pointer record → Telegram card edit. WP0 implementation must:

- make **"durably queued" the commit point**, and treat the card's wording as a *retried projection* of receipt state, never the source of truth;
- make the governed Markdown write **idempotent** (check for an existing target/evidence before writing) so a resumed worker never double-writes;
- never let a card show "Completed" unless `written → evidenced` has actually occurred.

This is a delegated implementation constraint, not a Warwick decision.
