...
- Security (Vex WP0 delta review — identity binding, 2026-07-17): `Security/wp0-delta-review-identity-binding-2026-07-17.md` — **GREEN** (0/0/0/0); Codex **F-MED-DB-CODEX-PROVIDER-CHECK-NOT-PER-PRINCIPAL CLOSED** by 0002 exact per-principal CHECK (total over the 4-value principal enum); no prior control regressed (RLS deny-by-default, immutable 0001, honest seed, no-autonomous-merge all intact); delta commits 48679ea + 6832a2e.
- GPT-controller MEDIUMs (post-approve hardening, 2026-07-17): **MEDIUM-1 (durable external-write outbox) CLOSED** — `clickupPoster.js` rewired onto `ftw.external_write` (migration `0003`, Silas); per-mutation idempotency, claim-before-post, `applied_verified` requires a comment id, ambiguous-timeout reconcile by embedded `mutationId`; six outcomes proven on memory store + real Postgres (`clickupPoster.test.js`, `postgresStore.integration.test.js`). **MEDIUM-2 (no-relay proof on DURABLE Tower state) CLOSED** — `scripts/proof-norelay-ack.js` rewritten onto **PostgresStore** with a genuine dispatcher/store restart (create → close pool → recover from Postgres → real-`claude` signed ack → persist → reconnect → re-read), staged evidence read from the stable Tower-owned `Architecture/evidence/posted-review-readback.md`; DB-gated + claude-gated; [[larry-norelay-ack-2026-07-17]]. No live ClickUp/GitHub/Supabase writes performed.
