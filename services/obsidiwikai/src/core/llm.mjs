// LLM helper — structured (JSON) reasoning via the box (LightRAG /api/generate).
...
export async function generateJSON(prompt, { retries = 2 } = {}) {
  ...
  for (let i = 0; i <= retries; i++) {
    ...
    const raw = await lightrag.generate(prompt + suffix);
...
export async function generateText(prompt) { return lightrag.generate(prompt); }
