...
//
// FAIL-CLOSED: a missing file, an unreadable file, malformed/absent frontmatter, or
// a status other than `approved` blocks the turn (an unapproved governing prompt
// must never drive a live review).
 ...
 * { ok, text, fingerprint, version, status, error }. Fingerprint is
 * sha256(file bytes) — the prompt_fingerprint recorded on every verdict.
 */
export function loadQaSkill({ path, fs = fsDefault } = {}) {
  ...
  if (status !== 'approved') {
    return { ok: false, error: `fail-closed: QA skill status is "${status ?? '(none)'}" — must be "approved"`, fingerprint, status, version };
  }
  return { ok: true, text, fingerprint, version, status, error: null };
}
