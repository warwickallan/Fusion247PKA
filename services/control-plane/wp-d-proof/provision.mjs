...
async function main() {
  ...
  try {
    ...
    console.log('[provision] seeding SYNTHETIC data (010 review log, 020 shopping, 030 read-models, 040 write-back seam)…');
    ...
    await client.query(fs.readFileSync(path.join(SEED, '040_writeback_seam.sql'), 'utf8'));
    ...
    const counts = await client.query(`
      select
        ...
        (select count(*) from public.command_request) as command_requests,
        (select count(*) from public.cockpit_metric)  as cockpit_metrics`);
