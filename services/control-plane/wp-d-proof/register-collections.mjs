...
const collections = [
  ...
  // increment 2 — the write-back seam.
  ['command_request',  'send', 'WP-D write-back seam: INTENT queue. Cockpit inserts requested-only rows; a trusted worker executes + receipts.'],
  ['cockpit_metric',   'calculate', 'WP-D write-back seam: worker-computed synthetic metrics (read-only in the cockpit).'],
];
try {
