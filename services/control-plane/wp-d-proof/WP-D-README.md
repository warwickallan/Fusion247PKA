...
> **Increment 2 is now built** — see `WP-D-INCREMENT-2.md` (constrained check/uncheck,
> the `command_request` → trusted-worker → receipt seam, least-privilege `cp_directus`/
> `cp_worker` DB roles, and the Directus-outage independence proof). One-command runner:
