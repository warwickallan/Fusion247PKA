...
// REAL headless `claude` when invocable; the Codex turn is PINNED to a recorded
// blocker (record-blocker mode) so this synthetic proof never spends live ChatGPT
// quota — the real Tower-owned Codex turn is proven separately (WP0 step 6b).
// Emits ONE terminal notification and writes a masked transcript to
// Builds/BUILD-010-fusion-tower/Architecture/wp0-synthetic-proof.md.
//
...
async function main() {
  ...
  // Real Larry adapter (live if claude is invocable). The Codex adapter is PINNED
  // to record-blocker here: this host now HAS an authenticated codex binary, so an
  // 'auto' turn would spend live ChatGPT quota. The synthetic proof stays free of
  // live spend; the real Tower-owned Codex turn is proven separately (WP0 step 6b,
  // codex-review-draft-2026-07-17.md).
  const larry = createLarryAdapter({ config, cwd: __dirname, mode: claudeProbe.invocable ? 'auto' : 'record-blocker' });
  const gpt_codex = createCodexAdapter({ config, cwd: __dirname, mode: 'record-blocker' });
...
function writeTranscript({ config, trail, verdict, sink, notices }) {
  ...
  lines.push('printed. The Larry turn runs a REAL headless `claude`; the Codex turn is PINNED');
  lines.push('to a recorded blocker (record-blocker) so no live ChatGPT quota is spent — the');
  lines.push('real Tower-owned Codex turn is proven separately (codex-review-draft-2026-07-17.md).', '');
  lines.push('## Verdict', '');
  lines.push('```json');
