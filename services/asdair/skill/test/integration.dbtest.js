...
// ---------------------------------------------------------------------
// SAFETY GUARD: refuse to run against anything that could be live.
// Only host / db-name are inspected; the credentials in the URL are never
// read out or logged. Throws (loud failure) on an unsafe target -- pointing
// this test at live Supabase MUST fail, never silently proceed.
// ---------------------------------------------------------------------
function assertSafeDbTarget(rawUrl) {
  ...
  try {
    ...
    throw new Error('ASDAIR_DB_URL is not a parseable URL; refusing to run the integration test.');
  }
