...
export async function createLiveRunner(config, opts = {}) {
  ...
  // durable event write.
  //
  // NOT the WP4 decision-card grammar: that one is `decision:<card_id>:<key>` and
  // does NOT start with the four bytes `dec:`, so the two callback vocabularies are
  // disjoint and the WP4 filer path below is unreachable from here (and vice versa).
  ...
  async function handleUpdate(update, now) {
    ...
    // through to the UNCHANGED capture behaviour below — which still includes the
    // WP4 decision-reply / decision-tap paths, whose grammars are disjoint from
    // the governance grammar (see handleGovDecision).
