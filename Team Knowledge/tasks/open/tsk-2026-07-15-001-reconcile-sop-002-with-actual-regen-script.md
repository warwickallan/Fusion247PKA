...
- 2026-07-28 09:48 (larry) — recovered from stranded PR #24 (`claude/ws-004-team-retro-recovery`) during estate/PR hygiene. Re-verified still valid: `SOP-002-convert-mypka-to-sqlite.md` on current `main` still describes the phantom `mypka_to_sqlite.py` prompt-script; `Expansions/mypka-cockpit/scripts/regen-mypka-db.py` is still the real, unchanged mechanism. The drift this task names is unresolved and current, not stale. `updated` timestamp bumped to reflect this re-verification; `created`/`source` left as the true original record.
