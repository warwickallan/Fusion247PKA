...
- **Triggered by:** natural-language phrasing that signals "the team should learn from itself" or "run the retro." See **Trigger contract** below. The close-session routine also runs the Tier 1 check every session and MAY nudge the Tier 2 retro on a roughly monthly cadence as an option (never automatically). **Implementation note (added 2026-07-12):** the nudge is now delivered as a standing scheduled Routine (fires into a fresh session, independent of whether any myPKA session happens to be open) rather than left to chance inside an ad hoc close-session check — this exists because "does anyone remember to nudge" was itself a real gap. The Routine only ever *asks* whether to run the retro; it never runs Tier 2 or writes a proposal document on its own. The hard invariant is unchanged: the retro never fires on its own, and nothing it produces is applied without the user's explicit approval. **Cadence: weekly** (Warwick's explicit override, same day, given the current intensive build pace — WS-004's own stated default above remains "roughly monthly" for calmer periods; adjust the Routine again if the pace changes). Routine ID `trig_01TE2BaDNGvcsd2dBfZSaDch`, fires Mondays 09:00 UTC into a fresh session — Warwick can delete/adjust it directly if the cadence or approach ever needs to change.
