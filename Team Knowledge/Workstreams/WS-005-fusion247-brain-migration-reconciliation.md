...
**Correction (2026-07-15, per Warwick's explicit instruction, after PR #23's first pass conflated routing with semantic merge):** a row's disposition is evidence that the row was *routed and dispositioned* — it is never, by itself, proof that a source's meaning was *semantically merged* into myPKA. `absorbed`, `mapped-to-existing`, and similar completion-implying labels apply only where direct evidence supports them (a genuinely re-read source this pass, or a source on Warwick's explicit confirmed whitelist of already-checked agent/orchestration capabilities). Everything else gets an honest routing outcome instead: historical/defunct operating material superseded by git and this repo's own governance mechanisms; source material routed to a named Idea or Build for a later, separately-authorised comparison (not claimed merged in the act of routing); or a pilot/reference bundle that never required migration into governance. Future reconciliation passes may audit remaining unread bundles by logical grouping (e.g. Bundle 8's seven-document migration-evidence audit, [[2026-07-15-build-000-bundle-8-direct-audit]]) rather than rereading all 84 sources at once. Only unresolved material that could affect current myPKA governance gets implemented during a BUILD-000-style pass; routing material to its correct future Build or Foundry idea is a complete, sufficient outcome on its own and does not require implementation now.
