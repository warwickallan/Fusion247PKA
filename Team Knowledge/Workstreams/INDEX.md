...
| WS-005 | [[WS-005-fusion247-brain-migration-reconciliation]] | Pax (primary executor) + Larry (provisioning + synthesis) + Silas/Warden (consulted on domain judgment calls) | Scoped, source-led audit of how much of a specific prior system (Fusion247 Brain) has an equivalent home in myPKA. Reads the source's own precedence-ordered registers (Drive Object Registry, master index, decision log, work list, session log, implementation plan) and cross-references against this repo's git history, producing a Migration Coverage Matrix with a disposition per capability. Report-only — never migrates or implements. References [[tsk-2026-07-10-001-fold-fusion247-brain-doctrine-into-warden]], [[GL-006-client-delivery-frontmatter-conventions]]. |
