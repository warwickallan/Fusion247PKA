...
Your myPKA grows. New fields will surface. Three acceptable extension paths:
...
2. **Add a new entity type under `PKM/`.** Higher cost. Requires a new folder under `PKM/`, a new schema in this Guideline, a new section in [[SOP-002-convert-mypka-to-sqlite]], and a new template in [[Templates/INDEX]]. Do not do this casually.

3. **Add a new entity type for a domain that is structurally separate from `PKM/`.** Path 2 assumes the new entity lives under `PKM/` and feeds the SQLite mirror in SOP-002. When a domain is deliberately kept outside `PKM/` (its own top-level root, its own eventual dashboard — e.g. `Client Delivery/`), do not stretch this Guideline's scope or SOP-002's PKM-only folder map to cover it. Instead, create a sibling Guideline (next free `GL-NNN`) scoped explicitly to that root. The sibling Guideline `[[wikilink]]`s back here for the shared mechanics (§§2-4: field naming, typing rules, foreign-key-stores-the-slug) rather than duplicating them, and defines its own entity schemas, its own template set, and its own answer to "does this feed a SQLite mirror" (default: no, unless a matching section is added to SOP-002 as a deliberate decision). See [[GL-006-client-delivery-frontmatter-conventions]] for the worked example.
...
- **v2.5** - Clarified this Guideline's scope boundary in §"How to extend this Guideline": added path 3 for domains structurally separate from `PKM/` (their own top-level root, not feeding SOP-002's PKM-only SQLite mirror by default) - these get a sibling Guideline that wikilinks back here for shared mechanics (§§2-4) instead of stretching path 2's "new folder under `PKM/`" language to fit. No change to any PKM entity schema. First sibling: [[GL-006-client-delivery-frontmatter-conventions]], covering `Client Delivery/`'s Engagement, Work Package, and Register Item entities for Warden.
