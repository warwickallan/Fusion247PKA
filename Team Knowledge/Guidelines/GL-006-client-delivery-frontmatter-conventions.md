...
- `status` includes `blocked` as a value on the enum, never a separate folder or file — same "blocked-state as metadata, not a folder" doctrine the team already follows elsewhere. Do not create a "Blocked Work Packages" folder. The same doctrine covers `cancelled` and `done`: a superseded or completed Work Package stays in `Work Packages/`, filterable by status — see §"Archive, intake, and UAT/training semantics" below for why this Guideline adds no Archive folder.
...
**Archive — no new folder; status values already are the archive, for both entity types.** F247's `11_Archive` stage held superseded/deprecated/rejected content per engagement. GL-006 does not add an equivalent folder, for two different reasons depending on what kind of file is in play:

- **Frontmatter-bearing entities (Register Item, Work Package).** A Register Item's `status: rejected | resolved | closed` and a Work Package's `status: cancelled | done` are already terminal, already queryable states (see §Entity schemas above) — this generalizes the "blocked-state as metadata, not a folder" doctrine the Work Package schema already states for `blocked` to every terminal status on both entity types. A superseded Work Package or a rejected/closed Register Item stays exactly where it is, filterable by status, with no physical move. Moving the file on a status change would also break slug-based foreign-key resolution (`linked_register_items`, `linked_work_packages`, per GL-002 rule 4) for zero benefit, since status already answers the question a folder move would be trying to answer.
- **Plain-markdown rollups** (Implementation Plan, Work Package Catalogue, Comms Plan, `Sources (Immutable)/INDEX.md`). These are living append/rollup documents by design (see the folder-map notes above) — wholesale replacement of the whole file is not part of their normal lifecycle, so no Archive folder is needed for them either, under normal operation. Archive-not-delete is still a hard doctrine here — it is the exact failure named in the live Fusion247 Brain incident behind Warden's Critical rule 8 (a README deleted, not archived) — but the fix is behavioral, not structural: never `rm` a `Client Delivery/` file outright, only edit or replace its content in place. Where local version history is enabled (the myPKA "time machine" git baseline, opt-in per `ADAPTER-PROMPT.md` §5), every prior version of these files is already recoverable there, which satisfies archive-not-delete without a dedicated folder. If a real engagement later shows a genuine need to preserve a whole superseded version of one of these rollups outside git, that is a real, separate schema request to Silas — not built speculatively here.
