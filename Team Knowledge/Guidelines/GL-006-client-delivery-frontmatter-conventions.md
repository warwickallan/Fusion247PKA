...
### 3. Engagement mnemonics for prose and documentation

Distinct from the file-path slug (rule 2 above), an engagement also gets a short, human-readable mnemonic for use in prose, deliverables, task updates, and session logs — e.g. `NPL`, `BCC`, `UHP`, `UOR`. Settled by Warwick, 2026-07-12, after an earlier documentation pass had wrongly invented an anonymizing placeholder (`BRK-001`) for one on the mistaken assumption that the code itself needed hiding for GL-009 public-repo compliance:

- **Sensible, three-character, human-readable, derived from the organisation or project name.** Not a random or anonymizing code — chosen so a reader immediately recognizes which engagement is meant.
- **Larry asks Warwick where the mnemonic is ambiguous** (e.g. two engagements could plausibly share initials); otherwise Larry may choose an obvious, unique three-character mnemonic himself and record it here or in the engagement's own note.
- **A numeric suffix is added only where multiple engagements need distinguishing**, e.g. `NPL-001`, `NPL-002` — not by default for a single engagement.
- **The mnemonic itself is not private.** GL-009's public/private boundary and this Guideline's own privacy expectations continue to apply in full to people, confidential evidence, contractual details, meeting content, and other sensitive operational records — never to the three-character code that names the engagement. A committed file may say `NPL` freely; it may not say who NPL's contacts are, quote their words, or reproduce their contractual terms without the same care any Client Delivery content already requires.
- **This does not change or replace the file-path slug** in rule 2 above (e.g. `bellrock-npl-implementation`) — that slug is a mechanical, GL-001/GL-002-governed identifier for foreign-key resolution across the whole root. The mnemonic is the separate, shorter form used when talking *about* the engagement rather than linking *to* one of its files.

### 4. One entity type per file, same as PKM
...
- **v1.5** - Engagement-mnemonic naming convention (Warwick's explicit clarification, 2026-07-12). Added **§3 "Engagement mnemonics for prose and documentation"**, settling a question an IDEA-003 evaluation pass had gotten wrong: an earlier documentation-only pass ([[2026-07-12-client-delivery-operational-schema-evaluation]] and its task/session-log trail) had invented an anonymizing placeholder (`BRK-001`) for the live NPL engagement, on the mistaken assumption that a Client Delivery engagement's own short code needed hiding for GL-009 public-repo compliance. Warwick corrected this directly: three-character, human-readable mnemonics derived from the organisation or project name (e.g. `NPL`, `BCC`, `UHP`, `UOR`) are approved and not private; only a numeric suffix (`NPL-001`) is added, and only where more than one engagement needs distinguishing; Larry asks Warwick where a mnemonic would be ambiguous, otherwise picks an obvious unique one himself. Privacy controls are otherwise unchanged — people, confidential evidence, contractual details, meeting content, and other sensitive operational records are still fully protected; only the three-character code itself was ever wrongly treated as sensitive. Renumbered the existing "One entity type per file" rule from §3 to §4; no other rule renumbered or changed. No entity schema, field, or folder changed by this pass — documentation and naming-convention only.
