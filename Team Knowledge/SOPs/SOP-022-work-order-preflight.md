...
## Ordinary dispatch route — generation is not optional (J1-1)

**Canonical here. Closed 2026-08-06 as part of BUILD-020 Phase 4.**

Every Work Order issued to a specialist **must** be produced by `node tools/wo/envelope.mjs` (the ordinary dispatch route). The emitted file carries the provenance header marker `GENERATED by tools/wo/envelope.mjs`.

**Worker rule at read-back (class A):** if the order body does **not** contain that marker, **REFUSE** with the reason *order was not produced on the ordinary generation route*. Hand-authoring is not an escape hatch except where Warwick has explicitly authorised a one-shot exception in the order itself (`hand_authored_exception: Warwick, YYYY-MM-DD — reason`). Machine-install orders use `--machine-surface` on the generator (G-6); they are not hand-authored by default.

**Issuer rule:** do not draft envelope fields from memory. Run the generator, fill bare AUTHOR REQUIRED slots, recompute readiness with `node tools/wo/envelope.mjs --count-markers <file>` (G-5), then dispatch. This is the route, not a remembered optional command.

**What this is not:** a new checker service, registry or control plane. The existing read-back gate is the teeth.
