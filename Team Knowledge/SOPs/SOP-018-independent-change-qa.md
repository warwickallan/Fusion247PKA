...
- **Default owner:** **Veritas** for integrated build work; **Pax** for a commissioned or exploratory audit, when Larry scopes the request. **This SOP still does not create a specialist** — it is a skill, the portable successor to Fusion247 Brain's old `/update QA` skill, and any specialist can run it when they need to independently verify what was claimed vs. what was actually built.
- **Amended 2026-08-04 (`GOVERNANCE-VERITAS-HIRE`) — read this before treating the header above as current.** This SOP previously read *"This SOP does not create a new specialist ... not a permanent agent"*, with Pax as sole default owner. **A permanent internal assurance specialist now exists:** [[Team/Veritas - Internal Quality and Truth Assurance/AGENTS]]. The two are not in competition and neither absorbs the other:
  - **Veritas is a standing gate.** It fires on defined events — an integrated Work Package, a phase or vertical-slice boundary, a change to an accepted decision/runtime/route/boundary — and a Work Package cannot be recorded complete without its `VERITAS_PASS` against the exact integrated head. It reviews the **integrated head**, not a claim.
  - **SOP-018 is a callable procedure.** It answers *"did the build match the claim"* for any scope, on request, run by anyone. Veritas's method draws on it; running it does not make the runner Veritas, and running it never substitutes for the gate.
  - **Pax is no longer the routine internal QA department.** He returns to research and to audits Warwick commissions. *(Carve-out: for BUILD-015 only, Pax remains the already-authorised sole final acceptance gate.)*
  - **Codex is unchanged** — the external QA authority at PR and release level, who additionally checks whether Veritas performed the internal assurance role properly.
...
- **Does not create a new specialist or persona.** It is a callable procedure. Anyone can run it. **It also does not substitute for the Veritas gate** — running this SOP over integrated build work produces a report, not a `VERITAS_PASS`, and a Work Package is not complete because someone ran a procedure over it.
