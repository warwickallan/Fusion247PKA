...
| SOP-022 | [[SOP-022-work-order-preflight]] | Every worker executing a bounded Work Order | Verify the order against observable reality **before** implementing — commands, paths, baselines, env vars, *which datastore*, permissions, internal consistency, authoritative contracts. Materially wrong or ambiguous → REFUSE / ESCALATE / PARTIAL. Never game a criterion. The first phase of execution, not a review ceremony. |

*Claimed 2026-07-27:* **SOP-021** by the weekly AsdAIr shop method (rescued out of machine-local memory into Git as part of BUILD-015); **SOP-022** by Work Order Preflight (Warwick's ruling after the IDEA-017 delegation experiments — a worker verifies the order against observable reality before implementing).
