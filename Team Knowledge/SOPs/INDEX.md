...
| SOP-021 | [[SOP-021-run-the-weekly-asdair-shop]] | Asdair (Larry holds the browser drive) | Run the weekly household shop end to end: intake → plan against the durable rulebook and Regulars → ask once → the proven ASDA Regulars bulk-add pass → reconcile → **record the outcome** → **learn**. Holds the expensive lessons (the stepper trap; out-of-stock kills a whole bulk add). Never substitutes, never books a slot, never checks out. |
| SOP-022 | [[SOP-022-work-order-preflight]] | Every worker executing a bounded Work Order | The mandatory Work Order lifecycle — `DRAFT → WORKER READ-BACK → LARRY ACCEPTS OR AMENDS → ISSUED → RUNNING`. **Phase 1 read-back verifies understanding and is the gate: no implementation begins until Larry accepts it, and work produced without one is returned REFUSED on process grounds.** Phase 2 preflight verifies reality — commands, paths, env vars, *which datastore*, permissions on **both** halves of a boundary, internal consistency, the caching/distribution layer the order forgot, authoritative contracts. Larry owes a reply; silence is not consent. Artefact shape: [[Templates/work-order]]. |

*Claimed 2026-07-27:* **SOP-021** by the weekly AsdAIr shop method (rescued out of machine-local memory into Git as part of BUILD-015); **SOP-022** by Work Order Preflight (Warwick's ruling after the IDEA-017 delegation experiments — a worker verifies the order against observable reality before implementing). *Extended 2026-07-29* on Warwick's direct instruction: the worker read-back became a mandatory **dispatch gate** ahead of the preflight checks, after a session in which workers caught six-plus material defects in Larry's own orders but caught them at completion rather than before the build.

*Reserved (genuinely open for future agents):* SOP-023 onward. SOP-003–009 were claimed by the v3.0.0 all-in-one bundle (App Developer Pack → 003–005, Designer Pack → 006–009); SOP-010–014 claimed by Warden's meeting-intelligence buildout (2026-07-10); SOP-015 claimed by Cairn's own pilot-processing buildout (2026-07-11); SOP-016 claimed by Cairn's transcript chunk-mapping buildout (2026-07-10, reconciled from the Fusion247 Brain precedent); SOP-017 claimed by the content-integrity QA capability (2026-07-11, per tsk-006); SOP-018 claimed by the independent-change-QA capability (2026-07-11, per tsk-2026-07-11-001); SOP-019 claimed by Larry's Fusion delivery-tracking buildout (2026-07-12, per Warwick's explicit authorization); SOP-020 claimed by the Fusion 247 Handbook currency discipline (2026-07-15, per Warwick's explicit instruction after Handbook acceptance). Do not back-fill below SOP-021 without coordinating across the team.
