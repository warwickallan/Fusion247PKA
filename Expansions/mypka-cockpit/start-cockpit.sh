#!/usr/bin/env bash
# start-cockpit.sh — myPKA Cockpit launcher (Linux). Run: ./start-cockpit.sh
# Generated locally from launcher/templates/linux.sh.txt — review before use.
set -euo pipefail

# --- config (the LLM fills these in for THIS machine) ------------------------
PORT="${PORT:-4317}"
# export MYPKA_ROOT="/absolute/path/to/your/myPKA"   # only for a non-standard layout
...
if db_has_core; then
  # FAST PATH — the DB is already here. Refresh from markdown if Python+PyYAML are
  # available (non-destructive); otherwise just serve the existing DB.
  ...
  if ! command -v python3 >/dev/null 2>&1 || ! python3 -c "import yaml" >/dev/null 2>&1; then
    ...
    echo ""
    echo "      • Python 3:  your distro's package (e.g.  sudo apt install python3 python3-pip)"
    echo "      • PyYAML:    pip3 install --user pyyaml   (or  sudo apt install python3-yaml)"
    echo ""
...
if command -v lsof >/dev/null 2>&1 && lsof -ti "tcp:$PORT" >/dev/null 2>&1; then
  ...
  fuser -k "$PORT/tcp" 2>/dev/null || true   # fallback when lsof is absent
fi
...
echo "Starting the cockpit on http://127.0.0.1:$PORT/  (Ctrl-C to stop it)"
