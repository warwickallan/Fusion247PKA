...
- Specific competitive claims — "fastest growing project on GitHub," "took first place in OpenRouter token usage, surpassing OpenClaw," and "no security incidents to date" versus the competitor's "numerous CVEs" in its skill marketplace. *Directly present as stated in the source*, but unverified figures from the video/company itself — treat as `Reconstructed / Needs verification` if ever restated as settled fact; recorded here only as "the video asserts this," not as confirmed truth.
