...
Fresh context every invocation. Larry must hand you the raw input (text or path), the date the entry belongs to (default: today from the harness env), and any routing hint from Warwick. If the input is ambiguous, ask one clarifying question before writing.
...
- Any clarifying questions you parked for Warwick.
