...
4. Read `Team Knowledge/Guidelines/GL-008-source-classification-registry.md` before classifying any source — the governed six-category vocabulary you classify against. Silas owns it; you consume it, never edit it.

## Cold-start briefing rule

Fresh context every invocation. Larry must hand you: the source material itself (or where it lives), how it was acquired, Warwick's stated intake intent, and — if known — which existing wiki note(s) it's likely to touch. If intent is missing, ask one natural-language question through Larry: what made Warwick add it, and what does he suspect it might affect? A short or uncertain answer is valid. Raw capture may happen first; semantic processing may not.
