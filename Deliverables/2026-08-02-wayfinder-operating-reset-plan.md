...
- **Phase 5 — teardown review candidate — NOT STARTED (Warwick decision recorded 2026-08-02; execute on NEXT resume, not the recording session).**
  - **Decision (Warwick):** prepare the **FULL** review candidate — BUILD-018 spine deletion (ledger §C) + **proposed** CLAUDE.md thinning (§F) + **proposed** footer-set bin (Decision D) — as ONE reviewable branch diff. **Do NOT integrate.**
  - **Constraints:** (i) **preserve the proven continuity behaviour until any replacement is runtime-proven** — do NOT delete `reorient.mjs` or re-point SessionStart until a replacement is proven live to inject the brief as SessionStart `additionalContext` JSON (`continuity.mjs read` currently emits plain text, not that JSON — fix + prove first); (ii) make **capability losses explicit** — the loose-`Deliverables/` sweep AND programme-state recovery that `reorient.mjs` performs, plus footer telemetry; (iii) do NOT modify any `AGENTS.md` (hard rule) — flag dangling refs to deleted modules for Warwick; (iv) obtain **independent Codex/Tower review of the real diff against this Wayfinder, run twice/repeatable**, before returning for the integration decision.
  - **Exact next action (resume here):** [1] verify `origin/main` head, cut branch `operating-reset/teardown`; [2] runtime-prove a brief-injecting SessionStart replacement BEFORE retiring `reorient.mjs`; [3] surgical §C spine deletions + retire/narrow `governor-tests.yml`, as a branch diff (nothing landed); [4] add proposed CLAUDE.md thinning + footer-set bin to the diff; [5] write the explicit capability-loss list; [6] Codex/Tower `mergeCheck.mjs` ×2 against the Wayfinder; [7] present reviewed diff + losses to Warwick for the Phase 6 integration decision — integrate NOTHING until his yes.
