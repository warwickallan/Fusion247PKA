...
| 10 | 🔴 **The context denominator guard is INSUFFICIENT — my spec, not Keel's build.** `resolveWindowTokens` rule 2 borrows a statusLine-observed window from any prior sample whose **model id matches**. But `claude-opus-5` runs at BOTH 200k and 1M depending on session config, so model id does not determine window size. Proven live 2026-08-02: `used_tokens=401229` against a borrowed `context_window_size=200000` — usage exceeding the window, which is impossible. Would have rendered a confident, wrong percentage: the exact false green the repair exists to prevent. **Fix: reject any denominator where `used_tokens > context_window_size` (cheap, decidable, catches this class outright), and stop treating model id as sufficient authority.** | 🔴 Blocks trusting any rendered % |
| 11 | 🔴 **`deriveFooterFields` cannot render from a transcript-sourced sample** — returns `state: undefined`, so `renderFooter` throws `state must be one of GREEN \| AMBER \| RED \| RECOVERY \| BLIND`. Reproduced three times against this session's real transcript, with and without a denominator. **The footer repair is therefore NOT working end-to-end**, despite its unit tests passing (46,080 round-trip combinations) — those exercise the renderer from synthetic fields, never from a real `extractTranscriptSample` output. A seam no test crosses. This is why "unit tests pass" is not "the feature works". | 🔴 The repair is unproven in the one path that matters |
