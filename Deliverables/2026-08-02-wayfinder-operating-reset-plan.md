...
| 9 | ~~**`gatherGitEvidence` has no pathspec support**~~ **DONE `468d0c8`** — pathspec landed on both git calls, `scoped_to` recorded, unscoped proven byte-identical, 214/214 still pass. Original note:, so a review cannot be scoped to a subset of files and any diff over `MAX_DIFF_BYTES` (60k) is silently truncated. This is what blocked Phase 5's gate. A `--paths` flag was drafted in `reviewDiff.mjs` and **removed** on discovering the option would be silently ignored. Proper fix belongs in `gitEvidence.mjs`. | Blocks the Phase 5 gate — do this first on resume |
