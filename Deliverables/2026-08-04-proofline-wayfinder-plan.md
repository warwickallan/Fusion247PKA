...
| **3** | **Codex contract corrected** at `services/control-plane/review/prompts/tower-qa-skill.md` — merge-class review gains responsibility **B: will this merge actually converge the estate?** Stranded useful state is a **merge blocker**. Codex reports, never repairs. **The live loader must consume the amended contract** — an amended contract the runtime does not read is not done. | ✅ **LANDED** `6ae592c` + evidence route `9c14e5b`. Proven through `loadCodexContract()` — the LIVE route, not the retired `qaSkill.js`. **11,890 bytes of estate inventory proven to reach the child's stdin**; 75 subtests; every new control mutation-tested to fail and restored. ⬜ Wording rebase onto the outcome-bound model IN FLIGHT. |
| **4** | **The WHOLE estate converged** — every branch, worktree, stash, dirty file, untracked file, patch, rescue folder, installed runtime and scheduled task classified **by CONTENT, never by name**, to exactly one of **KEEP → canonical** or **DISCARD → dead**. | 🟠 **CLASSIFIED, NOT EXECUTED.** Full disposition in [[Deliverables/2026-08-07-4c-estate-disposition-kill-list]] (`b765b09`). **Warwick asked to review before any deletion** — destructive execution is held pending his word, and the permission layer independently denied `git worktree remove`. |
| **5** | **The named survivors dispositioned** — Pax's CAPAE report banked as a 4D input (no CAPAE analysis) · the asdair 34-line WIP resolved for estate purposes only · the unapproved VlogOps draft given a safe disposition consistent with its privacy status · old-location `tower-qa-skill.md` copies proven dead. | 🟠 **MOSTLY DONE.** ✅ asdair WIP **recovered** `a8c2a33` — `main` still carried the bug; 69/69 pass. ✅ `stash@{2}` deferred-hardening ruling recovered `d7967c6`. ✅ PKM position preserved **local-only per Warwick's decision**. ✅ Pax report + payload banked `70dba16`. ✅ old-location copies proven dead — absent from `main`, no live loader resolves them. ⬜ **VlogOps draft: durable local copy taken at `~/.mypka/unpublished-drafts/`; disposition is Warwick's.** |
| **6** | **End state proven MECHANICALLY**, not asserted — the fourteen checks in § 4C TARGET END STATE. | ⬜ **BLOCKED on row 4.** Cannot be attempted until the destructive half executes. |
