...
- **Status:** **RECOMMENDATION ONLY — nothing created, bound or activated.** No file was written under `Team/`, no shim under `.claude/agents/`, no row added to `Team/agent-index.md`, no git command run. SOP-001 §8 (`Team Knowledge/SOPs/SOP-001-how-to-add-a-new-specialist.md:127-129`) requires Warwick's approval before any of that, and Warwick has reaffirmed the gate.
- **SOP-001 step 2 (Pax research) status:** running in parallel, not yet folded in. Every section marked **[PAX-FOLD]** below must be reconciled with Pax's brief before Warwick sees a final version. Per SOP-001 §3 (`:37-39`) the *name* is supposed to be selected from Pax's candidates — my proposal below is a placeholder with reasoning, not a decision.
