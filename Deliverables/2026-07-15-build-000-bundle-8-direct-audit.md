...
| Model memory labelled reconstructed, needing verification | `verified-already-present` **(citation corrected 2026-07-16 — the original citation was tangential, per Fable's review)** | Root `AGENTS.md` Hard rule 2, **"Memory precedence"** ("Local file beats global memory... follow X" — reconstructed recall never outranks a checked source); `SOP-015-*.md` §7, **"Label every non-obvious claim by evidence origin"**; `GL-006-client-delivery-frontmatter-conventions.md`'s `evidence_type`/`confidence` fields (`inference` — "anything inferred rather than stated deserves a check before it is treated as settled"; `confidence: low` for an unconfirmed or reconstructed statement). `SOP-018` principle 4 (the original citation) is related — it governs declaring unavailable QA evidence — but these three are the operative controls for labelling reconstructed content specifically. |
...
| Finding | Outcome | Repository evidence |
|---|---|---|
| Document-specific defects tied only to the superseded Drive documents and Drive editing mechanism: master-index self-contradiction/stale promotion-era text (C/N-2), table-splice damage from a positional insert (C/N-3), stacked changelog blocks in `F247.agent.verifiair`/`F247.agent.registry` (C/N-4), stale "last updated" headers (M-1, M-2), and Drive-title trailing-whitespace (M-3) | `defunct/no-further-action` | `F247.master.index`, `F247.agent.verifiair`, `F247.agent.registry`, and `/Hey Fusion.md` are themselves already dispositioned `retained-as-source`/`mapped-to-existing` elsewhere in this ledger (rows 1, 2, 15) — not live myPKA control documents, so these document-specific defects have no myPKA target |
| **Correction (2026-07-16, per Fable's review):** the general-risk half — a duplicate, unregistered near-identical document pair going unnoticed (C/N-1), and the absence of a new-document registration check (IO-1) | `verified-already-present` | This is not eliminated merely by the superseded documents going away — a differently-named, unregistered duplicate document could exist in myPKA the same way. It is governed by standing controls: `SOP-018-independent-change-qa.md` Step 6's **"Duplicate active source-of-truth files"** and **"Duplicate instruction stores"** checklist items, plus `Team/Larry - Orchestrator/AGENTS.md` Duty 2's **orphaned-file detection** and **missing `INDEX.md` entries** check (new files that didn't get registered). Distinct from the same-path/index-splice classes above, which git and rendered indexes do eliminate structurally. |

---

## Summary across Bundle 8

**Correction history:** the first pass undercounted Document 1 (17 rows, not 16) and misclassified three findings. Fable's independent re-review of that correction (2026-07-16) found one further residual over-broad classification (Document 7) and a tangential citation (Document 5 finding 2), both fixed below. Every number in this section is recomputed directly from the finding tables above, not asserted.

- **7/7 sources directly read in full**, not cited from prior evidence alone.
- **Distinct findings enumerated:** 17 (Doc 1) + 3 (Doc 2) + 5 (Doc 3, one finding split in two) + 6 (Doc 4, one finding split in two) + 5 (Doc 5) + 1 (Doc 6) + 2 (Doc 7, one finding split in two per Fable's 2026-07-16 review) = **39 distinct findings/recommendations given an outcome.**
- **`verified-already-present`:** 28 — all with a named, checked repository path (`manifest.json`, `validation-script.sh`, `CHANGELOG-MIGRATION.md`, root `AGENTS.md` (including Hard rule 2, "Memory precedence"), `SOP-rebuild-task-index.md`, `WS-004`, `SOP-close-task.md`, `DATA-CONTRACT.md`, `GL-005`, `SOP-001`, `SOP-015` §7, `GL-006`'s `evidence_type`/`confidence` fields, per-agent `journal/` templates, `mypka.db`, Larry's iron rule and Librarian Duty 2, Cairn's contract/GL-008/SOP-015/SOP-016, `GL-011`, `GL-008`, and `SOP-018`'s boundary-drift, duplicate-source/instruction-store, literal-output-reread, and structural-damage checklist items). Includes four corrections across two review rounds: the "agent/skill boundary drift" QA-rule finding (Doc 2 — already a standing SOP-018 checklist item), the general semantic-duplication-risk half of Doc 3's duplicate-register finding, the general structural-damage-risk half of Doc 4's PR-2 finding, and (2026-07-16) the general duplicate-unregistered-document-risk half of Doc 7's finding (governed by SOP-018 Step 6 + Larry's Duty 2, not eliminated by superseded documents going away).
- **`implemented-now`:** 1 — Doc 5's model/runtime-provenance finding. `agent_id` names the specialist persona, not the runtime/model that actually processed the source. Fixed by adding `runtime_host`/`model_id` fields to `Team Knowledge/SOPs/SOP-write-session-log.md` (the single SSOT location — not duplicated into SOP-017, SOP-018, or GL-002).
- **`defunct/no-further-action`:** 8 (document-specific Drive-hygiene defects tied to specific now-superseded documents, and the Drive-specific mechanisms — same-name/different-ID folder collisions, the Drive-move/merge protocol, the position-index text-insertion API — that don't exist in a git-versioned markdown tree at all).
