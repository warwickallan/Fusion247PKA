...
| **⛔ READ BEFORE ANY `@tower` COMMENT** | **`Deliverables/2026-08-17-tower-review-packet-recipe.md`** — how to get a PR through the Tower/Codex route. Six review rounds were wasted learning it and **none criticised a line of product code.** Deliver the review, never request one; the checkpoint comment IS the claim; every review is merge-class so stage §3b evidence up front; dispose every finding at THIS head; serialise PRs of one build; the diff truncates at 60k. |
