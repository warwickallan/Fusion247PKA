...
> **Recovery note (2026-07-28).** This document and the five approved changes it records were stranded on
> `claude/agent-count-kdved6` (never a PR) and then on its recovery branch PR #24 (`claude/ws-004-team-retro-recovery`,
> opened 2026-07-16, left unmerged), both hundreds of commits behind `main` by the time they were found. Recovered
> onto a fresh branch from current `main` after semantic comparison against present doctrine — full assessment in
> `Deliverables/2026-07-28-pr24-recovery-assessment.md`. All five landed as originally approved below, **except**:
> proposals #1 and #3 target `Team/Larry - Orchestrator/AGENTS.md`, which has since been substantially rewritten
> by the 2026-07-27 doctrine reconciliation and tonight's CI-truth doctrine (§8a) — so only their two specific
> sections were extracted and re-applied, not the file as a whole, to avoid regressing newer doctrine. The
> `mypka.db` regeneration described in this retro's Step 6 was **not** repeated in the 2026-07-28 recovery — see
> the assessment for why.
