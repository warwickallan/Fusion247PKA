...
| 4 | ❌ **THIS ROW WAS WRONG — CORRECTED BY WARWICK, 2026-08-02.** I claimed the PR ⇄ Tower seam "was deliberately never built". **It was built and merged.** WO-OR-22 (`73bab3f`) delivered `ingestComment.mjs`, `db/comment_schema.sql` and the finding-disposition wiring; WO-OR-24 (`8a40219`) delivered `pollPrComments.mjs`, the real GitHub→ingest hop. **What is missing is only the permanent automatic trigger** — the poller is a one-shot and nothing continuously invokes it. `services/control-plane/tower-loop/README.md` § "Known limits" says so in its own words: *"Something must still invoke the poller."* **My error:** I investigated the LEGACY BUILD-010 ClickUp watcher (`services/tower-baton/`), found no GitHub input path there, and generalised that to the whole estate — a negative claim asserted without checking the newer implementation, which is the one failure mode I have a standing rule about. Two Tower implementations exist and I conflated them. → **Superseded by the migration; see the Tower watcher migration plan.** | Corrected on the record, not quietly rewritten |
