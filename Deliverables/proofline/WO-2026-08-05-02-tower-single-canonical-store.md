...
| **file_surface** | *(v1 list — **Amendment 1 EXTENDS it**; read Amendment 1 for the operative surface)* `services/control-plane/tower-loop/mergeCheck.mjs` · `services/control-plane/tower/merge-check.mjs` · `services/control-plane/tower-loop/apply.mjs` · `services/control-plane/tower-loop/db/` (new schema file) · `services/control-plane/tower-loop/db.mjs` (**only** if `TIMESTAMP_COLUMNS` registration requires it — Keel confirmed it did **not**) · `services/control-plane/tower-loop/test/**` |
| **acceptance_property** | ~~end-to-end against `~/.mypka/tower/tower.db`~~ — **SUPERSEDED by Amendment 1.** v1 demanded a write to the live store that `live_authority: none` forbids. **See Amendment 1** |
| **private_surface** | 🚨 ~~`C:\.fusion247\` — READ ONLY, and only `tower-baton.env` / `control-plane-dev.env`~~ — **STRUCK. This was a GL-012 BREACH: it declared the secrets-store ROOT.** The correct value is **`none`** — this work needs no access to that store at all. **Amendment 1 governs. Do not act on this row.** *(Left visible rather than deleted: this is the defect the Phase 3 report must count, and erasing it would erase the evidence — but it is struck because a live order must not read as authorising it. Reported by Keel as still-present, 2026-08-05; fixed same day.)* |
