...
**T-04 (Opus), T-10 (Opus).** T-01, T-02, T-03, T-07, T-09, T-13 all resolved 2026-07-31
(see §10 write-back).

**T-03 and T-13 landed together**, as predicted: they shared no file
(`tools/governor/sampler.mjs` vs `tools/governor/collect-state.mjs`) and no git state
beyond the branch, so they ran safely in parallel. **T-10 no longer waits on anything** —
every ticket in its `depends_on` (T-07, T-09, T-13) is resolved, so it is fully unlocked.
T-04 is also unlocked. Both are genuine Opus-tier judgement (state-space design;
refuse-or-proceed rotation safety), not mechanical adapter work — the Sonnet-tier
implementation frontier this map anticipated is now exhausted.
...
3. **Wave 3 (current frontier, Opus):** **T-04** (needs T-03 ✅) · **T-10** (needs T-13 ✅, fully unlocked)
