...
**T-11 (Opus), T-04 (Opus).** T-01, T-02, T-03, T-07, T-09, T-10, T-13 all resolved
2026-07-31 (see §10 write-back).

**T-11 is the next action** — reorientation. T-10 landed the banking half of the rotation
loop (state is banked, pushed, and the canonical handoff is written); T-11 closes the loop
by getting that handoff into the *fresh* session automatically via
`SessionStart(source="clear")` → `additionalContext`, ≤10,000 chars. Until T-11 lands, the
rotation works but the fresh Larry must be pointed at the handoff by hand — which is
precisely the re-briefing this build exists to remove.

**T-11 carries two known live constraints, both already recorded** — `X-2` (the settings
file is gitignored, so a hook ticket that only edits a file in this worktree ships
nothing) and `Q-5`/§12 (a broken `ensure-watcher.mjs` SessionStart hook already fires
every session, and the map says decide what happens to it *before* adding a Governor hook
beside it).

T-04 (pure evaluator) remains unlocked and independent — it is the *advice* half of the
build, not the rotation path, so it does not block T-11.
