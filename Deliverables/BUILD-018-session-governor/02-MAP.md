...
| **AD-26** | **THE ESCALATION RULE — settled 2026-07-31 by Warwick, binding.** When a harmless **no-action** option preserves correctness and avoids additional risk, Larry takes it **automatically** and does not ask. **Never escalate:** typos, wording, formatting, naming, cosmetic metadata, ticket boundaries, completed workers, ordinary routing choices, or anything else Larry can safely resolve — and **never offer a force-push or history rewrite merely to improve cosmetic metadata.** **Escalate only for:** a genuine product decision, a material scope change, an unavoidable permission, spend, an irreversible live action, an unsafe repository state, or the final merge decision. **This governs `AskUserQuestion` exactly as it governs ordinary stopping.** | An unnecessary escalation is not a neutral courtesy — it is an **acceptance failure** that spends Warwick's scarce attention and stalls the autonomous execution this whole build exists to protect. Warwick's own reasoning for making it **mechanical rather than advisory**: *"A Stop hook alone is insufficient if Larry can manufacture a pause by asking an unnecessary question"* — so the control must bind the **ask** surface, not just the **stop** surface. Recorded as a decision rather than left to judgement because **the failure is silent and looks virtuous in the moment**: an unnecessary question reads as diligence, which is exactly why it recurs. The live instance (**D-4**) had Larry offering to rewrite **correct, already-pushed history** to fix a stray character in a commit subject. |
...
**Completed (14):** T-01, T-02, T-03, T-04, T-05, T-07, T-09, T-10, T-11, T-13, T-14, T-15, T-16, T-17
**Frontier — takable now (4):** T-06 [Opus], T-08 [Opus], T-12 [Sonnet], T-18 [Sonnet]
**Resumption:** T-18 — model Sonnet
...
| **T-05** | ~~**Status-line display: render verdict compactly.**~~ **RESOLVED 2026-07-31.** | Sonnet | T-04 ✅ | ✅ met — `tools/governor/status-line.mjs`: `renderStatusLine` (pure, AD-11) + `computeStatusLine` (the one impure wrapper); all five states rendered from real `evaluate()` output and asserted pairwise distinct; degrades gracefully over missing `advice`/`unknownSignals`, empty `reasons`, and `null`/`undefined`/non-object verdicts. 14/14 ticket tests, **528/528** suite-wide | ✅ passing — evaluator throws → line still renders and shows **BLIND**, proven for both a thrown `Error` and a thrown non-`Error` value; `assert.doesNotThrow`, and the line never reports GREEN/AMBER/RED/RECOVERY |
...
| **T-19** | **Adopt the shared primitive in the remaining three sites**, and correct the false concurrency comment (**D-6**). `build-registry.mjs:275`, `programme-state.mjs:630` **and** `:638` (the banking path), `qa-binding.mjs:288`. Found by Keel's T-18 read-back (C4). | Sonnet | **T-18** | All five sites use one primitive; the comments state what is actually true | A comment claiming a guarantee the code does not provide is itself the defect — an absent claim must not simply replace it |
| **T-18** | **Windows-safe atomic write** — bounded retry on `EPERM`/`EBUSY`/`EACCES` around the rename, temp-file cleanup in a `finally`, fixed in the ONE shared primitive rather than patched into two modules. Registered 2026-08-01 from **D-5**, found by Larry's independent verification of T-17 — **not** by any ticket's own tests. | Sonnet | — | Concurrent writers succeed; the proof asserts a **non-zero success count**, not merely the absence of tearing | A forced `EPERM` is retried and succeeds; a permanently-failing rename still **never orphans** a temp file, and never reports success |
| **T-17** | ~~**Git-lifecycle escalation gate**~~ **RESOLVED 2026-08-01.** — refuse an escalation whose options offer Warwick a decision **AD-20** already assigns to Larry (force-push, amend of a pushed ref, rebase/reset/filter of pushed commits). Covers the **ASK** surface (`AskUserQuestion`), not only ordinary stopping. Added after Warwick ruled a cosmetic-typo escalation an acceptance failure (**D-4**, **AD-26**). **RE-SCOPED at Keel's CLARIFY read-back** — see §10. | **Opus** | — | The D-4 shape classifies REFUSED **with no "is cosmetic" input supplied** (the refusal must stand on AD-20 alone); detection by an exported, enumerable git-verb vocabulary, never free-text intent parsing; closed-enum escape hatch for the genuine unsafe-repo-state case | **Gate throws / malformed input / unrecognised shape / UNCLASSIFIABLE → the escalation is ALLOWED** (fail open, AD-6/INV-2 — over-blocking a legitimate escalation is the worse defect); plus a positive control proving a genuinely-warranted escalation (merge, spend, irreversible live action) is never refused |
...
| **T-16** | ~~**Mechanical delegation-enforcement gate**~~ **RESOLVED 2026-07-31.** Delegation ledger (Silas-owned schema) + substantial-work threshold on direct Write/Edit/MultiEdit/mutating-Bash, composed additively onto `worktree-guard.mjs`'s PreToolUse pattern; a `task` checkpoint records `governing_specialist`/`specialist_match` (mechanical exact-match against `.claude/agents/*.md` only — `Team/agent-index.md` domain-matching stays out of v1 scope). Added mid-programme after this session bypassed Keel/Silas for T-15's own design and implementation (see D-2, D-3). | **Opus** | — | ✅ met — implemented under an accepted Keel Work Order (WO-2026-07-31-01, SOP-022); `tools/governor/delegation-gate.mjs` + `delegation-gate.test.mjs` (60/60) + `delegation-ledger-record.schema.json`; 514/514 suite-wide, independently re-run by Larry | ✅ passing — ledger read/write forced to throw → ALLOW, never DENY (discipline gate, not safety-critical); `worktree-guard.test.mjs` 27/27 identical before and after, file byte-unmodified |
...
| 2026-07-31 | T-16 | **RESOLVED — the operating-model correction closes with this ticket.** Added mid-programme after Warwick caught this session bypassing the named specialist team for T-15's own design and implementation (D-2), and Nolan's follow-up audit found that bypass was not isolated — the entire programme (T-01–T-14) had the same provenance (D-3). Correction, in order: Larry's routing cheatsheet fixed (`Team/Larry - Orchestrator/AGENTS.md`, explicit direct approval from Warwick, applied by Larry after Nolan correctly refused to act on a *relayed* claim of that approval — the exact "no self-edit of core rules on relayed authority" failure mode, caught working as intended); SOP-001 fixed (new mandatory step, "definition of done" checklist, this exact failure named under common mistakes); Silas decided the ledger's schema (`Deliverables/BUILD-018-session-governor/decisions/D-delegation-ledger-schema.md` — discriminated union by `kind`, JSONL over qa-binding.mjs's JSON-array pattern reasoned through rather than defaulted to, physical-line-order checkpoint counting, `governing_specialist`/`specialist_match` mechanically populated by exact-match against `.claude/agents/*.md` only, and a genuine correctness bug caught and fixed in the original draft's fail-open reasoning); Keel implemented it under a proper accepted Work Order (WO-2026-07-31-01) — **routed via the general-purpose runtime instructed to operate under Keel's actual written contract verbatim, because the harness does not expose `keel` as a directly-dispatchable subagent type this session despite the shim being committed** (a live infrastructure gap, flagged plainly, not silently worked around or hidden). 60/60 new tests (up from an earlier unauthorised draft's 34, held as reference only and not accepted), `worktree-guard.mjs` untouched (27/27 identical before/after, byte-identical diff), 514/514 suite-wide, independently re-run by Larry, not just the builder's self-report. Labelled honestly by the builder: "Builder self-test evidence — NOT independent review" — SOP-018 QA has not run over this diff, the same standing pre-merge requirement as every other ticket in this programme, not a new gap. | Frontier reverts to the pre-correction set (T-05/T-06/T-08/T-12); T-05 remains the chosen next action, now explicitly noted as itself a Keel-domain ticket for whoever resumes next |
| 2026-08-01 | T-17 | **RESOLVED — and the read-back gate paid for itself a second time, in the ORDER rather than the work.** Keel returned **CLARIFY, not ACCEPT**, and was right twice. (1) The order asked the gate to decide whether a defect was *cosmetic*; Keel identified that as **semantic in disguise** — no enumerable signal exists for it, and a caller-supplied field asserting it would be **self-attestation by the very actor who wants to escalate**, "a hope, not a control" in `delegation-gate.mjs`'s own words. (2) Asked directly whether the UNCLASSIFIABLE bucket would swallow every real case, Keel said **yes, plainly**, rather than diplomatically. Both answers changed the ticket instead of being noted and built past. **The amendment re-grounded the gate on AD-20** — *Larry owns the complete git lifecycle; Warwick never manages it* — which deletes the judgement step entirely: offering Warwick a force-push choice is wrong **regardless** of what sits behind it, because that choice was never his. AC3 then required the D-4 test to pass **no "is cosmetic" input at all**, so the refusal must stand on AD-20 or fail. 60 new tests, **588/588** suite-wide. **Larry verified by ADVERSARIAL PROBE against the real module, not by re-reading the builder's tests:** the actual D-4 question shape (its verbatim options, *"Leave it"* vs *"Amend + force-push"*) → **REFUSED** citing AD-20; three genuinely-warranted escalations (merge decision, spend, irreversible live action) → **all ALLOWED**, never wrongly blocked; five malformed inputs (`null`/`undefined`/string/`{}`/number) → **all UNCLASSIFIABLE**, none threw, none refused; the closed-enum escape hatch cleared the refusal for `unsafe-repository-state` while an arbitrary string (*"because I said so"*) **did not** — the hatch cannot be talked past. `worktree-guard.mjs`/`delegation-gate.mjs` byte-unmodified. **Coverage stated honestly and enforced by AC6:** this detects ONE mechanically-decidable shape and **does not enforce AD-26 generally** — typos, wording, formatting, naming, ticket boundaries, completed workers and ordinary routing have no enumerable signal and **remain unenforced**, still resting on Larry observing AD-26 unaided. Narrowness accepted **knowingly**. **NOT ACTIVATED:** `install-hooks.mjs` untouched, so the gate has no live effect until a separately-authorised step; the unproven `AskUserQuestion` PreToolUse payload shape is recorded as fog for it. | **D-5 and T-18** — Larry's verification of this ticket is what surfaced a real, measured concurrency defect **pre-existing in T-02/T-16**. T-18 takes precedence over T-12 |
| 2026-08-01 | **D-5 (defect found, not a ticket)** | **A CONTROL THAT REPORTED SUCCESS OVER GROUND IT NEVER EXAMINED — found only because verification went past re-running the builder's own tests.** Keel honestly reported one delegation-gate subtest failing on a rerun and called it a one-off flake. It was not: run in isolation it fails **roughly 2 runs in 5**. Root cause is **not** in T-17 — it is a **shared defect in `health-store.mjs` (T-02) and `delegation-gate.mjs` (T-16)**, which both do `writeFileSync(tmp)` then `renameSync(tmp, target)` **with no retry**. On Windows that rename throws `EPERM` whenever another process holds the target. **Measured, not inferred: 21 of 48 writer processes failed under 16 concurrent processes — 44%.** **The evidence claim that does not survive:** T-02's note cites a "12-process concurrent-write mutation test" as proof. What that test actually established is that the file is never **torn** — true, rename is all-or-nothing. It never established that concurrent writers **succeed**; they frequently do not. Classic *state what was PROVEN, not what it implies*. **Why the suite hid it:** the failure is probabilistic, and **588/588 green** simply meant this run got lucky. **Secondary defect:** every failed rename **orphans its temp file permanently** — 21 accumulated in the real health store during the probe and were removed by hand. **Direction of harm is safe but not nil, which is why it is a defect and not a curiosity:** a lost health sample degrades to BLIND/stale (AD-3) and a lost delegation checkpoint undercounts toward a *missed* deny — but **T-03's sampler is meant to run on every assistant message from the live statusLine**, exactly the high-concurrency path this primitive fails on, so the real-world hit rate rises sharply the moment that wiring is activated. | **T-18** registered to fix it, ahead of T-12 — T-12 extracts the portability adapter over these very modules, and extracting a known-broken primitive would copy the defect into the adapter boundary |
| 2026-07-31 | T-05 | **RESOLVED — and the first ticket in this programme built end-to-end under the corrected operating model.** `tools/governor/status-line.mjs`: `renderStatusLine(verdict)` is pure (verdict object in, string out; the only import is `./evaluator.mjs`, asserted by a structural source-scan test that names `node:fs`/`node:child_process`/`node:process` and every stateful estate module as explicitly absent), and `computeStatusLine(signals, { evaluateFn = evaluate })` is the single impure composition function. **The AD-11 split is now proven twice over** — the renderer is exercised with hand-built verdict objects without `evaluate()` ever being called, *and* through the real integration path. **The map's named mutation test passes in both directions of failure**: a thrown `Error` and a thrown non-`Error` value both render a BLIND line rather than crashing or reporting a healthier state (INV-1, AD-3). Method note, since the throw had to be *manufacturable*: `evaluate()` is proven total over its documented domain by T-04's own 64-combination sweep, so it cannot be made to throw naturally — `evaluateFn` is injectable purely so the control can be **made to fail**, following `model-gate.mjs`'s `readSample = readHealthSample` precedent rather than monkey-patching an ESM named export. **One false-positive caught by the builder in its own test, recorded rather than quietly fixed:** an early AC2 assertion banned the substring `"GREEN"` from the rendered line and failed — because the real BLIND advice text legitimately contains the words *"never report GREEN"* as part of its own warning. Replaced with a check on the line's actual reported state via its `GOVERNOR <STATE>` prefix. **This is the third time in this programme a substring-shaped control has been the wrong control** (T-10's INV-4 ban, AD-25's argv-shape correction, now this) — the pattern is worth naming: *ban the behaviour, never the word.* 14 new tests, **528/528** suite-wide (was 514/514), zero regressions. **Process:** built by Keel under Work Order **WO-2026-07-31-02** (`tickets/T-05-status-line-work-order.md`), read-back returned and explicitly ACCEPTed before any file was written — the read-back named four points (fixture-reuse reading, injectable `evaluateFn`, synthetic-BLIND-on-throw construction, single-line vs. `model-gate.mjs`'s multi-line precedent), all four confirmed in writing first. **Independently re-verified by Larry, not accepted on the builder's self-report:** tests re-run (14/14, 528/528), the surface-scoped secret scan re-run (`--surface`, exit 0, 3/3 declared files), and the commit diff reconciled — 3 files, **0 outside `file_surface`**. Still labelled "Builder self-test evidence — NOT independent review": Larry's re-run confirms the evidence is real, it is **not** SOP-018 independent QA, which remains a standing pre-merge requirement for every ticket in this programme. **Deliberately not done:** not wired into the live `statusLine` command (T-03's deferral precedent — production activation needs its own Warwick-gated dispatch), and no live signal gathering (T-12's scope). | Frontier is now **T-06 / T-08 / T-12**. `deriveResumption` correctly refused to auto-select among three — **Larry chose T-12** as the next action: it is Sonnet-tier, carries no open prototyping fog, and Larry's own recorded Q-3 recommendation already favours designing the adapter boundary now ("nearly free" under AD-11). **T-06 is not structurally deprioritised** — it is Opus-tier and genuinely blocked on judgement inputs (F-5's prompt-detection prototype and Q-2's override design, both still open for Warwick), so spending Opus on it before those land would be premature. T-08 (dogfood) stays last per §9's own wave order |
