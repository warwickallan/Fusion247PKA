...
**T-04 (Opus), T-14 (Opus).** T-01, T-02, T-03, T-07, T-09, T-10, T-11, T-13 all resolved
...
**T-14 is the next action** — the layer above: a build-session registry/launcher keyed on
the build *name*, automatic programme-PR create/update at merge readiness, exact-head QA
binding, and presenting Warwick only the final merge decision. It is specified in full at
`tickets/T-14-build-session-registry-and-merge-flow.md` and was deliberately **not**
implemented inside T-11.

T-04 (pure evaluator) remains unlocked and independent — it is the *advice* half of the
build, not the rotation path, so it does not block T-14.
...
| **T-04** | **Pure evaluator** — `evaluate(signals) → verdict`. All five states, distinct exit codes, missing-field semantics. **UNLOCKED.** | **Opus** | T-03 ✅ | Full state-space unit tests incl. every `unknown` combination | **Delete the state file → asserts BLIND, not GREEN** (INV-1). Assert non-zero count of signals actually examined. |
...
| **T-14** | **Build-session registry/launcher by build NAME** + automatic programme-PR create/update at merge readiness + **exact-head QA binding** + present Warwick only the final merge decision. **Specified by T-11, deliberately not implemented there.** | **Opus** | T-10 ✅,T-11 ✅ | Warwick names a build and nothing else; PR is created/updated by Larry (AD-20), idempotently; every review verdict bound to the exact reviewed SHA and marked superseded when the head moves | Move the head after review → the prior verdict must NOT carry forward; unknown head is never "the reviewed head" |
