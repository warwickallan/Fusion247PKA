...
| **T-17** | **Mechanical escalation gate** — refuse/flag a Warwick escalation that a safe no-action default already resolves. Covers the **ASK** surface (`AskUserQuestion`), not only ordinary stopping. Added mid-programme after Warwick ruled a cosmetic-typo escalation an acceptance failure (see **D-4**, **AD-26**). | **Opus** | — | Escalation warranted by AD-26's enumerated list passes untouched; a cosmetic/safely-defaultable one is refused or flagged with the reason | **Gate throws → the escalation is ALLOWED** (fail open, AD-6/INV-2 posture — over-blocking a legitimate escalation is the worse defect); plus a positive control proving a genuinely-warranted escalation is never blocked |
