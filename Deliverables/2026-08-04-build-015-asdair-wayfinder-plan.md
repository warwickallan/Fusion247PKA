...
| | |
|---|---|
| **Outcome** | ✅ **Mum's basket delivered.** ⚠️ **CORRECTED 2026-08-18 — the figure this row carried has no artefact behind it.** The only committed evidence of the trolley is the executor's own read at **21:20:57.867Z**: **£132.52 · 55 items · 35 distinct products** (`basket.order_total`/`item_count`/`product_count` in `…-basket-run-log.json`, carried verbatim into `…-trolley-reconciliation.md`). This row previously read **56 items, £135.02** while citing those two artefacts as its evidence. That figure appears in no artefact; its only other occurrence is a borrowed fixture in `basket.test.cjs:401`, in a test asserting that a total and item count can never open the announcement gate. **It is NOT deleted** — the phase closed an hour after the executor's read and it may be the checkout total Warwick actually paid, which no committed artefact records. **Provenance UNESTABLISHED; the trolley-derived figure is the evidenced one.** The live trolley is **evidence and history only** — do not add, remove, re-run or reopen any line of it. |
