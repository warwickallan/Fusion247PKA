...
| 2 | When to move existing F247 docs into new folder structure? — `Open` | `retained-in-Drive` | n/a | No | Drive-internal sequencing question, overtaken by events. |
...
| 5 | Work/client meeting-transcript template content? — `Open` | `retained-in-Drive` | n/a | No | Warden's `SOP-010`–`014` solve the myPKA-side equivalent independently; this specific question is about a Fusion247-Drive template. |
...
| 8 | Drive→Markdown/Obsidian/Git migration timing? — `Deferred` | `approved-deferred` | this repo | No — in progress, not blocked | The register's own deferral trigger ("wait until laptop/local environment available") has been met; this audit's existence is the answer. |
...
| **Project template-pack reconciliation** | T028 above | — | `retained-in-Drive` | — | No | See B1. |
...
- **AsdAIr Microsite App** — `retained-external`, by the source's own explicit, pre-existing decision. Not a merge blocker.
- **F247-T028** — `retained-in-Drive`; a reconciliation internal to Fusion247's own competing drafts, not a Git obligation.
- Most of the Open Register's folder-naming/duplicate-cleanup questions — `retained-in-Drive`.
